    ...
    if request.method == 'POST':
        ...
        if state == 'start':
            ...
            if user_input.lower() == 'back':
                ...
                # Split the input into lines but don't add to history again
                ...
                if not names:
                    ...
                    # Remove the previous line that added the input to history
                    history.pop()  # Remove the f">>> {user_input}" that was added earlier
                    
                    # Store the names without displaying them again
                    session['remaining_names'] = names
                    session['converted_names'] = []
                    session['converted_aliases'] = []
                    session['state'] = 'processing_multi_add'
                    # Start processing without adding to history again
                    return alias_converter()
...
def process_dropped_duplicates(file_path):
    ...
    dropped_and_added_all_df = process_dropped_and_added_codes_all(df)
    dropped_and_added_strict_df = process_dropped_and_added_codes_strict(df)
    ...
    # Define the styles at the top level so they can be used in multiple places
...
def extract_category_from_filename(filename: str) -> str:
    ...
    name = name.replace('_', ' ')
    # Remove any trailing numbers or parentheses (e.g., (1), _1)
    name = re.sub(r'[\s_]*\(?\d+\)?$', '', name)
    # Use regex to extract category before 'Code Diff'
    match = re.match(r'^(.*?)(?:\s+\d+)?\s*Code Diff', name, re.IGNORECASE)
    if match:
...
def concept_summary():
    if request.method == 'POST':
        ...
        # Dictionary to hold summaries for each category
        category_summaries = {}
        ...
        try:
            for file in files:
                if file and allowed_file(file.filename):
                    ...
                    # Initialize summaries for the category if not already present
                    ...
                    if 'additions' in selected_options:
                        ...
                        with open(additions_output, 'r') as f:
                            additions_content = f.read()
                            category_summaries[category]['additions'].append(additions_content)
                    ...
                    if 'removals' in selected_options:
                        ...
                        with open(removals_output, 'r') as f:
                            removals_content = f.read()
                            category_summaries[category]['removals'].append(removals_content)

                    # Process Code Relocations if selected
                    if 'relocations' in selected_options:
                        # Ensure code_removals has been run
                        code_relocations(file_path, removals_output)

                else:
                    error = f"Invalid file type for file: {file.filename}. Please upload an Excel file."
                    return render_template('concept_summary.html', error=error)

            # Combine summaries into one output file
            ...
            with open(combined_output_path, 'w') as combined_file:
                for category, summaries in category_summaries.items():
                    combined_file.write(f"{category}\n")
                    
                    # Process additions
                    additions_contents = summaries['additions']
                    # Filter out empty additions
                    additions_contents = [content.strip() for content in additions_contents if content.strip()]
                    if additions_contents:
                        combined_file.write("The following concepts were created or modified and codes were added as described:\n")
                        for content in additions_contents:
                            combined_file.write(f"{content}\n")
                    
                    # Process removals
                    removals_contents = summaries['removals']
                    # Filter out empty removals
                    removals_contents = [content.strip() for content in removals_contents if content.strip()]
                    if removals_contents:
                        combined_file.write("The following concepts had codes removed as described:\n")
                        for content in removals_contents:
                            combined_file.write(f"{content}\n")
